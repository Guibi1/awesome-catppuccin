...
-- @param[type=Proxy] proxy a proxy object
-- @param[type=string] name the name of the property
-- @param[type=GVariant] the value to set
local function set_property(proxy, name, variant_value)
  -- proxy._proxy:set_cached_property(name, variant_value)
  return call(proxy, "org.freedesktop.DBus.Properties",
    "Set",
    GVariant("(ssv)", { proxy.interface, name, variant_value })
  )
end
...
local function generate_fields(proxy)
  ...
  if not xml_data_str and err then
    error(
      string.format(
